 ...
 * Task 02
 *
 * Put all elements in given range in ArrayList
 * NOT FINISHED!!!
 *
 */
public class AVLTree2<E extends Comparable<E>> {
    ...
    // inclusive
    ArrayList<E> countNodesInRange(E rangeFrom, E rangeTo) {
        ArrayList<E> res = new ArrayList<>();
        countNodesInRange(root, rangeFrom, rangeTo, res);
        return res;
    }

    void countNodesInRange(AVLNode<E> node, E rangeFrom, E rangeTo, ArrayList<E> res) {

        if (node == null)
            return;

        if (node.info.compareTo(rangeFrom) == 0) {
            //return 1 + countSmaller(node.right, rangeTo, res);
        } else if (node.info.compareTo(rangeTo) == 0) {
            //return 1 + countBigger(node.left, rangeFrom, res);
        } else if ((node.info.compareTo(rangeFrom) > 0) && (node.info.compareTo(rangeTo) < 0)) {
            //return 1 + countBigger(node.left, rangeFrom, res) + countSmaller(node.right, rangeTo, res);
        } else if (node.info.compareTo(rangeFrom) < 0) {
            //return countNodesInRange(node.right, rangeFrom, rangeTo, res);
        } else if (node.info.compareTo(rangeTo) > 0) {
            //return countNodesInRange(node.left, rangeFrom, rangeTo, res);
        }
        // never reaches here
        return;
    }

    int countSmaller(AVLNode<E> node, E rangeTo, ArrayList<E> res) {
        if (node == null)
            return 0;
        ...
        if (node.info.compareTo(rangeTo) == 0) {
            ...
            score += countSmaller(node.right, rangeTo, res);
            return score;
        } else if (node.info.compareTo(rangeTo) > 0) {
            return countSmaller(node.left, rangeTo, res);
        }
    ...
    int countBigger(AVLNode<E> node, E rangeFrom, ArrayList<E> res) {
        if (node == null)
            return 0;
        ...
        if (node.info.compareTo(rangeFrom) == 0) {
            ...
            if (node.right != null)
                score += node.right.total;
            score += countBigger(node.left, rangeFrom, res);
            return score;
        } else if (node.info.compareTo(rangeFrom) < 0) {
            return countBigger(node.right, rangeFrom, res);
        }
